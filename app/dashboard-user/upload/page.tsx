...
import { Upload, X, FileText, Copy, Trash2, CheckCircle, AlertCircle, Eye, MoreVertical, Zap } from "lucide-react"
...
const saveFileMetadata = (fileName: string, fileSize: number, fileContent: string, level: ExpiryLevel): string => {
    ...
    if (levelData.months !== null) {
        ...
        expiryDateString = "Seumur Hidup"; // Untuk Unlimited
    }
...
const PreviewModal = ({ file, onClose }: PreviewModalProps) => {
    ...
    const getFileType = (fileName: string) => {
        ...
        // File biner lain yang dibaca sebagai teks/dataurl (PPTX/DOC)
    ...
    const isHtmlDocx = fileType === "html" // DOCX
    const isText = fileType === "text" || fileType === "other" // Teks murni, atau fallback biner

    // Deteksi jika konten adalah pesan error/peringatan
        ...
        return (
             <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
                 ...
                 >
                     ...
                     <p className="text-sm text-gray-600">Konten file tidak dapat ditampilkan. Ini mungkin karena **file terlalu besar** atau format file (seperti PPTX atau DOC lama) tidak dapat diurai di sisi klien.</p>
                ...
                <div className="flex-1 overflow-auto p-6">
                    ...
                    isHtmlDocx && file.content ? (
                        <div className="bg-gray-50 rounded-lg p-6 overflow-auto max-h-96 border border-gray-200">
                            {/* Menggunakan 'prose' dari Tailwind Typography untuk styling yang lebih baik */}
                            <div className="prose max-w-none break-words" dangerouslySetInnerHTML={{ __html: file.content }} />
                        </div>
                    ) : (
                        /* FALLBACK (untuk Text, DOC, PPT, atau Error/Warning) */
...
// --- KOMPONEN MENU AKSI (Tidak Berubah) ---
...
const FileActionsMenu = ({ file, onPreview, onCopy, onDelete, isOpen, onToggle }: FileActionsMenuProps) => {
...
export default function UploadPage() {
    ...
    // FUNGSI readFileContent DENGAN PERBAIKAN PENANGANAN MAMMOTH.JS
    const readFileContent = (file: File): Promise<string> => {
        return new Promise((resolve, reject) => {
            ...
            // 1. KASUS DOCX: Gunakan mammoth.js
            if (fileNameLower.endsWith('.docx')) {
                reader.onload = async (e) => {
                    try {
                        const arrayBuffer = e.target?.result as ArrayBuffer;
                        
                        // Konfigurasi untuk menangani gambar di dalam DOCX
                        const options = {
                        ...
                        resolve(result.value); // Mengembalikan HTML
                    } catch (error: any) {
                        console.error("Mammoth error (DOCX):", error);
                        // Jika GAGAL URAI DOCX (file rusak/tidak standar), kirim pesan error yang jelas
            ...
            // 2. KASUS PDF/IMAGE: Baca sebagai DataURL
            else if (file.type.startsWith("image/") || file.type === "application/pdf") { 
                reader.onload = (e) => resolve(e.target?.result as string)
                reader.readAsDataURL(file)
            } 
            // 3. KASUS FILE BINARY LAIN (DOC, PPT, PPTX) & TEXT: Baca sebagai teks mentah
            else {
                reader.onload = (e) => {
                    const content = e.target?.result as string;
                    // Tambahkan peringatan khusus untuk file biner (DOC/PPT/PPTX)
                    if (fileNameLower.endsWith('.pptx') || fileNameLower.endsWith('.ppt') || fileNameLower.endsWith('.doc')) {
                        ...
                        resolve(content); // Untuk file teks biasa
                    }
                }
    ...
    const handleUpload = async () => {
        ...
        if (selectedFile.size > 5 * 1024 * 1024) { 
             showToast("info", "File Anda besar. Preview mungkin gagal karena batasan penyimpanan browser (LocalStorage).")
        }
        ...
        try {
            ...
            setTimeout(() => {
                if (selectedFile) {
                    try {
                        const generatedLink = saveFileMetadata(selectedFile.name, selectedFile.size, fileContent, expiryLevel)
    ...
    return (
        <section className="space-y-8 mx-4 md:mx-0 py-8 pb-24 md:pb-10">
            ...
            <div className="rounded-2xl border-2 border-foreground bg-white p-8 shadow-md">
                ...
                {/* AREA FILE TERPILIH DAN TOMBOL AKSI */}
                {selectedFile && (
            ...
            {uploadedFiles.length > 0 && (
                <div className="rounded-2xl border-2 border-foreground bg-white p-8 shadow-md">
                    <h2 className="text-2xl font-bold text-foreground mb-6">📋 Riwayat Upload</h2>
